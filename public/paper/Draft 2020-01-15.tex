...
We spend our waking hours inundated in mass media. The internet, newspapers, radio, and television stations broadcast to us a constant stream of facts, beliefs, entertainment, and ideas---ideas that percolate in our heads, and, we argue, ideas that shape our own identities and the way we live our lives. In this paper, we examine the impact of Spanish Language Television (SLTV) on Hispanics, focusing our analysis on three domains---the professional, the educational, and the political. In each case, we find that SLTV induces changes in behavior that we can attribute (at least in part) to a stronger sense of identity; causality is established via a spatial regression discontinuity introduced by a regulation on television network protection.

There's good reason to believe that identity, as reinforced through mass media, has a large effect on the lives people lead. @CITE Olken finds that that radio and television decrease `social capital' in Indonesia, in line with @CITE Putnam's argument.  @CITE Yanigazawa-Drott (QJE 2014) shows that radio broadcasts in Rwanda contributed to the violence and genocide that took place in the 90s. \cite{@CITE DellaVigna, Kaplan} find Fox News.\footnote{Other examples of media affecting identity and personal beliefs include \cite{@CITE Jensen, Oster}, \cite{@CITE Gentzkow, Shapiro 2004}, , \cite{@CITE La Ferrara, Chong}, \cite{@CITE Kearney 2015}. For an overview, see \cite{@CITE DellaVigna La Ferrara 2015}}

When explicitly looking at the effect of television on Hispanics, \cite{oberholzer-gee_media_2009} demonstrate that the presence of Spanish language local news increases Hispanic voter turnout \cite{velez_tuning_2019}, who first developed the instrument used in this paper, show that SLTV leads to lower voter turnout rates. \cite{@CITE Trujillo Paluck} run an experiment testing trust in the government and the census based on a soap opera scene. We extend on this literature by expanding the scope to a national-level analysis, while simultaneously exploring outcomes beyond the political realm.

We focus on Hispanic consumption of television for several reasons. 
...
- Firms
\cite{@CITE Bjorvatin} negative ; \cite{@CITE Berg and Zia} better control -- but both explicit target shows with programming 
Difficult to get entrepreneurship/mixed:  \cite{@CITE Karlan Valdiva} \cite{@CITE Gine Mansuri} (vs. us!)


- Education
\cite{@CITE Winn 2002} \cite{@CITE Gentile et al} \cite{@CITE Zavodny} vs. \cite{@CITE Kearney, Levine}  Gentzkow and Shapiro 2008 Preschool

\cite{@CITE Cascio} - school segregation

- Politics
Gentzkow and Shapiro 2008 (Competition and Truth in News Market), Gentzkow 2006 on turnout ; Stromberg 2004 on policy outcomes ; \cite{@CITE DellaVigna, Kaplan}

- Identity
old school priming - \cite{@CITE Benjamin}, \cite{@CITE Benjamin}  , more organic \cite{@CITE bursztyn moral}
Long term factors that strengthen idetnity  - \cite{@CITE Bisin}, \cite{@CITE Atkin}, \cite{@CITE Bazzi} - based on intergroup 
