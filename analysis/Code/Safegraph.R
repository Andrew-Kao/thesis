...
write.csv(poi_pattern, "POI/POI_pattern_clean.csv")
...
om1 <- lm(ihs(visitors) ~ hispanic*inside*hispanic_food + origpcHisp + origLogInc + origLogPop, data=poi_pattern, subset = sector == 72)
om2 <- felm(ihs(visitors) ~ hispanic*inside*hispanic_food + origpcHisp + origLogInc + origLogPop | stateCounty, data=poi_pattern, subset = sector == 72)
om3 <- felm(ihs(visitors) ~ hispanic*inside*hispanic_food + origpcHisp + origLogInc + origLogPop | naics_code, data=poi_pattern, subset = sector == 72)
om4 <- felm(ihs(visitors) ~ hispanic*inside*hispanic_food + origpcHisp + origLogInc + origLogPop | stateCounty + naics_code , data=poi_pattern, subset = sector == 72)
stargazer(om1, om2, om3, om4, out = "../../Output/Regs/sg_3d_hispfood.tex", title="Differential Effect of TV on IHS(\\# Hispanic Visitors to non-Hispanic food) vs. non-Hispanic",
          omit.stat = c('f','ser'), column.sep.width = '-2pt', notes.append = FALSE,
          omit = c("Constant",'origpcHisp','origLogInc','origLogPop','SCH_TEACHERS_CURR_TOT',
                   'total_students','SCH_GRADE_G01Yes','SCH_GRADE_G06Yes','SCH_GRADE_G09Yes'),
          order = c('hispanic:inside:hispanic_food','hispanic:inside','hispanic:hispanic food','inside:hispanic food','inside','hispanic','hispanic_food'),
          covariate.labels = c('TV $\\times$ Hispanic $\\times$ Hispanic food','TV $\\times$ Hispanic', 'Hispanic $\\times$ Hispanic food',
                               'TV $\\times$ Hispanic food', 'TV Dummy', 'Hispanic', 'Hispanic food'), se= makeRobust4(om1,om2,om3,om4) , #coeftest(om1),  list(om1r,om2r,om3r)
          dep.var.labels = 'IHS(\\# Visitors)')
