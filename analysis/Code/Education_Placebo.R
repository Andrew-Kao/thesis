...
## TODO: dda that's not just the sample
...
for (i in 1:1000) {
  ...
  contourLEADist <- gDistance(new_circle,harass_spdf, byid = TRUE)
  ...
  placebo_tv <- harass %>%
    mutate(TV = contourLEAInterAll, minDist = contourLEAMinDist) %>%
    filter(minDist < 1)
  ...
  om1 <- feols(ihs(sch_satact) ~ TV*eth +
                 ...
                 total_students + SCH_GRADE_G01 + SCH_GRADE_G06 + SCH_GRADE_G09 | LEAID, cluster = c("LEAID","eth"), data=placebo_tv,
...
ggsave("../../../analysis/Output/graphs/placebo_contour_1.pdf")
...
ggplot(placebo_2_ns, aes(x = x, y = coeff)) +
  ...
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +
  scale_color_manual(labels = c("actual","p < .05"), values = c("blue","red"))
ggsave("../../../analysis/Output/graphs/placebo_contour_2.pdf")
...
ggplot(placebo_3_ns, aes(x = x, y = coeff)) +
  ...
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +
  scale_color_manual(labels = c("actual","p < .05"), values = c("blue","red"))
ggsave("../../../analysis/Output/graphs/placebo_contour_3.pdf")
